...
def game():
    #引入全局变量
    global frame_game,But_game,entry_game,run_button,run_detail_list,list_frame,xgame,list_box,run_text,run_history,run_detail,run_entry,num_inside,num_input,level   
    level = 5
    hard='normal'
    #定义动态字符串
    ...
    run_date1=tk.StringVar()
    run_date2=tk.StringVar()
    run_date3=tk.StringVar()
    ...
    #比较个数,和正确率
    def compare(x,y):
        global in_put,same,diff,right,out_num,xgame
        ...
        in_side=''.join(y)
        if in_put == in_side:
            return ['恭喜你，答对了！答案为'+str(in_side)+'\n正确率为'+str(right)+'%',True]
            xgame=False
        else:
            return ["有"+str(same)+"个数字相同，有"+str(diff)+"个位置相同\n正确率为"+str(right)+'%',False]
    ...
    #输入校验,插入历史列表
    def run_step():
        global num_inside,box_game,num_input,level,run_history,list_history,run_button,run_entry,entry_game,num,gamedate
        run_button.set('继续')#更改按钮名称
        num_input=list(str(entry_game.get()))
        
        #获胜判定
        def win():
            global run_detail,Lab_run,win,frame_game,gamedate
            Lab_run.config(font=('楷体',24),height=3,width=11,anchor='center')
            run_detail.set('游戏结束')
            #新增按钮
            Bgame=tk.Button(frame_game,text='保存游戏数据',relief='flat',bd=0,fg=白色,bg=黑色,activebackground=灰色,justify='center',command=save_date)
            Bgame.grid(padx=192,pady=158,ipadx=52,ipady=3,row=0,column=0,sticky='nw')
            #弹出窗口
            win_win=windows()
            win_win.win('游戏胜利',白色,180,100)
            win_win.text('YOU WIN !',45,17,红色)
            win.cut(win_win,win_win.destroy)
            win_win.enter('确认',0,0,黑色,红色,win_win.destroy)
            win_win.mainloop()
        if len(list_history.date)==1:
            print(list_history.date)
            if list_history.date[0][2][0]==100.0:
                win()
            else:
                pass
        elif len(list_history.date)==0:
                pass
        else:
            if list_history.date[len(list_history.date)-1][2][0]==100.0:
                win()
            else:
                pass
        ...
        if (''.join(num_input).isdigit() and len(num_input)==level) == True:
            ...
            if check(num,str(entry_game.get())):
                func_detail(compare(num_input,num_inside)[0])
                #刷新历史列表
                ...
                #缩进选择
                if len(list_history.date)<10:
                    space='    '
                elif len(list_history.date)<100:
                    space='  '
                else:
                    space=''
                    
                index_lab=str(list_history.date[num-1][1][0])+'.'+space+str(list_history.date[num-1][1][1])+'                    '+str(list_history.date[num-1][1][2])
                ...
                box_game.see(num)
                #加载分数
                gamedate=list_history
                #得分
                def date1():
                    global gamedate,num
                    gamedate.date[num-1].append([])
                    if num-1==0:
                        gamedate.date[num-1][3].append(0)
                        pass
                    else:
                        A=gamedate.date[num-1][1][2].split(' ')
                        B=gamedate.date[num-2][1][2].split(' ')
                        C_1=int(A[0])-int(B[0])
                        C_2=int(A[1])-int(B[1])
                        source=gamedate.date[num-2][3][0]
                        gamedate.date[num-1][3].append(source)
                        if C_1<0:
                            gamedate.date[num-1][3][0] +=-C_1
                        else:
                            pass
                        if C_2<0:
                            gamedate.date[num-1][3][0] +=-C_2
                        else:
                            pass
                    gamedate.date[num-1][3][0] += 1#计步分数
                    return gamedate.date[num-1][3][0]
                
                run_date1.set('得分：'+str(date1()))
                run_date2.set('步数：'+str(gamedate.date[num-1][1][0]))
                if num-1==0:
                    run_date3.set('时间：'+'0')
                else:
                    start_time=gamedate.date[0][0]
                    run_date3.set('时间：'+str(round(time.time()-start_time,1)))
            else:
    ...
    #界面初始化
    def fame_new():
        global But_game,Lab_run,box_game,xgame,Lab_run,entry_game,run_button,run_text,frame_game
        ...
        #历史列表
        box_game=tk.Listbox(frame_game,bd=0,width=25,bg=白色,highlightthickness=1,highlightbackground=浅灰色,highlightcolor=灰色,selectbackground=浅灰色,selectforeground=黑色,selectmode='single',takefocus=False,justify='left')#创建游戏历史列表
        ...
        #得分表
        def datist(textVar,x,width):
            lab_date=tk.Label(frame_game,bd=1,bg=白色,height=1,width=width,highlightthickness=1,wraplength=185,anchor='w',justify='left')
            lab_date.grid(padx=x,pady=126,row=0,column=0,sticky='wn')
            lab_date.config(textvariable=textVar,anchor='nw')#绑定动态字符串
        datist(run_date1,193,7)
        datist(run_date2,250,7)
        datist(run_date3,310,12)
        
    #游戏初始化
    def game_new(gameseed):
        global run_history,box_game,entry_game
        #信息栏初始化
    ...
    if xgame == False:
        ...
        print(num_inside)
        fame_new()#初始化界面
        game_new(num_inside)#初始化参数
    else:
        fame_new()#初始化界面
        game_new(num_inside)#初始化参数
        global gamedate,list_history#获取上次游戏数据
        for i in gamedate.date:
            if i[1][0]<10:
                space='    '
            elif 10<=i[1][0]<100:
                space='  '
                ...
                space=''
            index_lab=str(i[1][0])+'.'+space+str(i[1][1])+'                    '+str(i[1][2])
            box_game.insert('end',index_lab)
        box_game.see(len(gamedate.date))
        list_history=gamedate
...
#重新开始
def restart():
    global xgame,list_frame,num_inside,gamedate
    print(xgame)
    if xgame==True:
        frame_game.destroy()
        list_frame.remove(frame_game)
...
def save_date():
    global gamedate
    date=gamedate.date
    file=open('GameDate-'+'Goal-'+str(date[len(date)-1][3][0])+'-'+dt.now().strftime('%Y%m%d-%H%M%S')+'.ling','w')
    file.write('Game_Date.time:'+dt.now().strftime('%Y%m%d-%H%M%S')+' Now:'+str(time.time())+'\n'+str(date))
    file.close()
    restart()
